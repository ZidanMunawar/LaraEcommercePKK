...
@section('title', 'Buy Now Checkout - ZynHope Apparel')
            ...
            <div class="row justify-content-center">
                <div class="col-xxl-12">
                    <div class="breadcrumb__wrapper text-center">
                        <h2 class="breadcrumb__title">Checkout (Buy Now)</h2>
    ...
    <section class="checkout-area section-space">
        <div class="container">
            <form id="buyNowCheckoutForm">
                @csrf
                <input type="hidden" name="product_id" value="{{ $buyNowItem['product_id'] }}">
                <input type="hidden" name="quantity" value="{{ $buyNowItem['quantity'] }}">
                <input type="hidden" name="size_id" value="{{ $buyNowItem['size_id'] ?? '' }}">
                <input type="hidden" name="color_id" value="{{ $buyNowItem['color_id'] ?? '' }}">
                        ...
                        <div class="your-order"
                            ...
                            <div
                                ...
                                <div class="d-flex justify-content-between mb-2">
                                    <span
                                        style="font-weight: 600; color: #8B6F47;">{{ $buyNowItem['product_name'] }}</span>
                                </div>
                                <small class="text-muted">Qty: {{ $buyNowItem['quantity'] }}</small>
                                @if (isset($buyNowItem['size_name']))
                                    <small class="text-muted"> | Size: {{ $buyNowItem['size_name'] }}</small>
                                @endif
                                @if (isset($buyNowItem['color_name']))
                                    <small class="text-muted"> | Color: {{ $buyNowItem['color_name'] }}</small>
                                @endif
                                <div class="d-flex justify-content-between mt-2"
                                    style="border-top: 1px solid #e9ecef; padding-top: 10px;">
                                    <span style="color: #666;">Rp {{ number_format($buyNowItem['price'], 0, ',', '.') }} x
                                        {{ $buyNowItem['quantity'] }}</span>
                            ...
                            <div
                                ...
                                <div class="accordion" id="paymentAccordion">
                                    ...
                                    <div class="accordion-item" style="border: 1px solid #D4A574; border-radius: 6px;">
                                        <h2 class="accordion-header">
                                            <button class="accordion-button collapsed" type="button"
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            // ========== FORM SUBMIT ==========
            $('#buyNowCheckoutForm').submit(function(e) {
                e.preventDefault();
                ...
                $.ajax({
                    url: '{{ route('customer.buy.now.process') }}',
                    method: 'POST',
                    data: $(this).serialize(),
                    success: function(res) {
                        if (res.success) {
